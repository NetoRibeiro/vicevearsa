...
- **Skill catalog**: `https://github.com/netoribeiro/vicevearsa/tree/main/skills`
- **Skill format reference**: `skills/vicevearsa-skill-creator/references/skill-format.md`
...
For the full SKILL.md specification, see `skills/vicevearsa-skill-creator/references/skill-format.md`.
...
2. **Fetch SKILL.md from GitHub**:
   ```
   https://raw.githubusercontent.com/netoribeiro/vicevearsa/main/skills/<name>/SKILL.md
   ```
   - If fetch fails (404 or network error) → **ERROR**: "Skill '<name>' not found in the ViceVearsa catalog."
...
5. **Fetch additional files** (if the skill requires them):
   - If the SKILL.md frontmatter has `script.path` → fetch the script file from:
     `https://raw.githubusercontent.com/netoribeiro/vicevearsa/main/skills/<name>/{script.path}`
...
1. Check if the `vicevearsa-skill-creator` skill is installed:
   - Look for `skills/vicevearsa-skill-creator/SKILL.md`

2. **If installed** → read `skills/vicevearsa-skill-creator/SKILL.md` and follow its
...
3. **If not installed** → inform the user:
   ...
   `/vicevearsa skills` → Install → `vicevearsa-skill-creator`"
...
3. **For each non-native skill**:
   ...
   c. **Verify MCP configuration** (if `type: mcp` or `type: hybrid`):
      ...
      - If missing → **ERROR**: "Skill '{skill}' is installed but its MCP server is not configured. Run `/vicevearsa skills` → Install to reconfigure."
