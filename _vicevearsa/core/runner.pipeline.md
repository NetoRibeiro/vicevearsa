...
If a step's frontmatter contains `approval_needed: true`:

1. **After the agent completes the step and veto conditions pass** (if any), emit an approval request to the dashboard:
   ```
   [APPROVAL_REQUEST]
   {
     "type": "approval",
     "step": "{step id}",
     "agent": "{agent name}",
     "question": "{what needs approval — from step frontmatter `approval_question` field}",
     "context": "{summary of output — first 200 words of the agent's deliverable}",
     "requestedAt": "{ISO timestamp now}"
   }
   ```

2. **Present the output to the user** in the chat with a note that dashboard approval is pending:
   ```
   ⏳ Output complete. Awaiting approval on the dashboard...
   {agent icon} {Agent Name}: [PENDING_APPROVAL]
   ```

3. **Wait for dashboard decision** — the chat environment will receive one of:
   - `[APPROVAL_RESPONSE: approved]` — User clicked "Approve" on dashboard
   - `[APPROVAL_RESPONSE: revised]` with instruction text — User clicked "Revise" with custom instruction

4. **If approved**:
   - Inform user: `✅ Approved by dashboard`
   - Update dashboard state (see Dashboard Handoff section)
   - Continue to next step

5. **If revised with instruction**:
   - Inform user: `🔄 Revised: {instruction}`
   - Inject the instruction into the agent's context: `"The user's revision instruction: {instruction}"`
   - Re-execute the same step with the instruction (same agent, same iteration but with feedback)
   - After re-execution, check veto conditions again
   - If output is approved on dashboard, proceed
   - If revised again, max 1 additional revision attempt per approval gate (prevent loops)
...
2. Update department memory (`departments/{name}/_memory/memories.md`):
   ...
   **For approvals**: If any approval gates occurred during the run, append to the "Approvals" section (or create it if missing):
   ```markdown
   ## Approvals
   ...
   - Step: {step id}
   - Agent: {agent name}
   - Question: {what needed approval}
   - Decision: **approved** | **revised**
   - Response Time: {seconds from request to decision}
   {If revised:}
   - Revision Instruction: "{instruction text}"
   ```

   **For general learnings**: Also append to the "Key Learnings" or "Performance Notes" section:
   - What the user approved/rejected
   - Any new preferences detected during the run
   - Review cycle count and outcome
   - Performance observations (e.g., "approval typically required for angles > 3 per slide")

   **How to append safely**:
   1. Read the existing `departments/{name}/_memory/memories.md` file using the Read tool
   2. Locate the appropriate section header (e.g., "## Approvals", "## Key Learnings")
   3. Append the new content before any section that comes after (e.g., before "## Recurring Issues")
   4. Write the updated content back to the file using the Write tool
   5. Preserve all existing content and section structure
