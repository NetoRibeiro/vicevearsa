...
async function loadSavedIdes(targetDir) {
  try {
    const prefsPath = join(targetDir, '_vicevearsa', '_memory', 'preferences.md');
...
export async function update(targetDir) {
  ...
  // 1. Check initialized
  try {
    await stat(join(targetDir, '_vicevearsa'));
  } catch {
  ...
  try {
    currentVersion = (
      await readFile(join(targetDir, '_vicevearsa', '.vicevearsa-version'), 'utf-8')
    ).trim();
  } catch {
  ...
  const newVersion = (
    await readFile(join(TEMPLATES_DIR, '_vicevearsa', '.vicevearsa-version'), 'utf-8')
  ).trim();
  ...
  for (const id of availableSkills) {
    if (id === 'vicevearsa-skill-creator') continue;
