...
async function runList(targetDir) {
  ...
  console.log(`\n  Browse available agents at: https://github.com/netoribeiro/vicevearsa/tree/main/agents\n`);
}

async function runInstall(id, targetDir) {
  if (!id) {
    console.log('\n  Usage: vicevearsa agents install <id>\n');
    return false;
  }
...
async function runRemove(id, targetDir) {
  if (!id) {
    console.log('\n  Usage: vicevearsa agents remove <id>\n');
    return false;
  }
