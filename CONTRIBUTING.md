# Contributing to ViceVearsa

Você está contribuindo para algo maior que código.

ViceVearsa existe para liberar empresas do trabalho repetitivo. Cada skill que você cria, cada melhoria que você faz, permite que empreendedores e times dediquem seu tempo àquilo que máquinas não conseguem fazer: criatividade, relacionamento, decisão humana.
...
ViceVearsa é um framework de orquestração multi-agente. Ajudamos empreendedores e profissionais não-técnicos a criar times de agentes de IA que trabalham juntos automaticamente.
...
O vicevearsa roda com `npx vicevearsa init` e pronto. Essa simplicidade é intencional. Contribuições que vão em outra direção provavelmente não serão incorporadas - mas a gente sempre pode conversar. Abra uma [issue](https://github.com/netoribeiro/vicevearsa/issues) antes para alinhar.
...
- Reescrever `_vicevearsa/core/` - prefira estender via skills e agents
- Aumentar significativamente a árvore de dependências - cada dependência nova é uma barreira a mais
- Funcionalidades de desenvolvimento de software - o vicevearsa é para automatizar processos de negócio, não para criar apps
...
2. Rode `npx vicevearsa init` em uma pasta de teste para experimentar o fluxo
...
Skills são o principal ponto de extensão do vicevearsa. É a melhor forma de contribuir. Use a skill `vicevearsa-skill-creator` para criar e iterar:

```
/vicevearsa install vicevearsa-skill-creator
```
...
O formato completo do `SKILL.md` está documentado em [`skills/vicevearsa-skill-creator/references/skill-format.md`](skills/vicevearsa-skill-creator/references/skill-format.md).
...
O vicevearsa já vem com guias de copywriting, design de carrosséis, SEO e mais. Mas tem muito mais a ser coberto. Use a skill `vicevearsa-agent-creator`:

```
/vicevearsa install vicevearsa-agent-creator
```
...
Abra uma [issue](https://github.com/netoribeiro/vicevearsa/issues) com:
...
Antes de abrir um PR com uma feature nova, abra uma [issue](https://github.com/netoribeiro/vicevearsa/issues) para discutir a ideia. Isso evita trabalho duplicado e garante alinhamento com a direção do projeto.
...
```bash
git clone https://github.com/netoribeiro/vicevearsa.git
cd vicevearsa
npm install
node --test
```
...
- Abra uma [issue](https://github.com/netoribeiro/vicevearsa/issues) com sua dúvida
...
# Contributing to vicevearsa (English)
...
Before you start coding, ask yourself: **"Does this change keep vicevearsa simple for non-technical users?"**

vicevearsa runs with `npx vicevearsa init` and that's it. That simplicity is intentional. Contributions that go in a different direction probably won't be merged - but we can always talk. Open an [issue](https://github.com/netoribeiro/vicevearsa/issues) first to align.
...
- Rewriting `_vicevearsa/core/` - prefer extending via skills and agents
- Significantly increasing the dependency tree - every new dependency is one more barrier
- Software development features - vicevearsa is for automating business processes, not for building apps
...
2. Run `npx vicevearsa init` in a test folder to experience the flow
...
Skills are vicevearsa's primary extension point. This is the best way to contribute. Use the `vicevearsa-skill-creator` skill to create and iterate:
...
The full `SKILL.md` format is documented in [`skills/vicevearsa-skill-creator/references/skill-format.md`](skills/vicevearsa-skill-creator/references/skill-format.md).
...
vicevearsa ships with guides for copywriting, carousel design, SEO, and more. But there's much more to cover. Use the `vicevearsa-agent-creator` skill:
...
Open an [issue](https://github.com/netoribeiro/vicevearsa/issues) with:
...
Before opening a PR with a new feature, open an [issue](https://github.com/netoribeiro/vicevearsa/issues) to discuss the idea. This avoids duplicate work and ensures alignment with the project direction.
...
- Open an [issue](https://github.com/netoribeiro/vicevearsa/issues) with your question
