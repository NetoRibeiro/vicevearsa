---
name: vicevearsa-dev
description: "vicevearsa development checklist — verifies templates sync, package integrity, and distribution correctness."
---

# vicevearsa Development Checklist

You are running the vicevearsa-dev verification skill inside the vicevearsa repository.
Your job is to detect and report distribution issues before they reach users.

## How vicevearsa Distribution Works

Understand this before checking anything:

- **`templates/`** → Copied by `src/init.js:copyCommonTemplates()` during `npx vicevearsa init`
  and by `src/update.js` during `npx vicevearsa update`. Everything in `templates/` except
...
- **`templates/ide-templates/`** → IDE-specific files. Copied selectively based on user's
  ...
  `antigravity/`, `cursor/`) maps to one IDE choice.

- **`agents/`** (project root) → Predefined agent catalog, distributed via npm
  (`package.json files[]`). Auto-installed during `npx vicevearsa init` and new agents
  added during `npx vicevearsa update`. Protected from overwrites in
  `src/update.js:PROTECTED_PATHS`. Users can also install manually via
  `npx vicevearsa agents install`.
...
- **`skills/`** (project root) → Bundled skills catalog, distributed via npm
  ...
  during init. Users can also install manually via `npx vicevearsa skills install`.

- **`_vicevearsa/core/*`** → Framework core files. MUST have a mirror copy in
  `templates/_vicevearsa/core/*`. Any change to a core file that isn't synced to templates
  means `npx vicevearsa init` delivers STALE content to new users.

- **`skills/*`** → Bundled skills catalog. Two distribution sub-types:
  - **Multi-file skills** (have scripts/assets/agents dirs alongside SKILL.md, e.g. `vicevearsa-skill-creator`):
    also have a mirror in `templates/skills/*` so the full directory structure is copied during init.
    Check B verifies this sync.
  - **Single-file skills** (only `SKILL.md`, no subdirs, e.g. `vicevearsa-agent-creator`):
...
vicevearsa supports multiple IDEs. When a user runs `npx vicevearsa init`, they choose which IDE(s) to install for. Files are copied from two places:
...
|------|-----------|----------|
| SHARED | Applies to all IDEs equally | `_vicevearsa/core/`, `templates/` (excluding `ide-templates/`) |
...
| Antigravity | `templates/ide-templates/antigravity/` | `.agent/rules/vicevearsa.md`, `.agent/workflows/vicevearsa.md` |
| OpenCode | `templates/ide-templates/opencode/` | `AGENTS.md`, `.opencode/commands/vicevearsa.md` |
...
- ❌ Wrong: Edit `_vicevearsa/core/runner.pipeline.md` adding `if antigravity: run sequentially`
- ✅ Correct: Edit `templates/ide-templates/antigravity/.agent/rules/vicevearsa.md` with the sequential execution instructions

**Legitimate shared file edit:**
- User asks: "Add a new researcher agent type to the architect."
- ✅ Fine: Edit `_vicevearsa/core/architect.agent.yaml` because the change benefits ALL IDEs equally.
...
#### Check A: Core file sync (`_vicevearsa/core/**` changed)

For EACH changed file matching `_vicevearsa/core/**`:

1. Compute the expected template path: `templates/{same relative path}`
   Example: `_vicevearsa/core/runner.pipeline.md` → `templates/_vicevearsa/core/runner.pipeline.md`

2. Run diff:
   ```bash
   diff "_vicevearsa/core/{file}" "templates/_vicevearsa/core/{file}"
   ```

3. If diff shows differences:
   - **FAIL**: Report the file and show the diff summary
   - **Fix**: `cp _vicevearsa/core/{file} templates/_vicevearsa/core/{file}`

4. If template file doesn't exist:
   - **FAIL**: "Template missing for `_vicevearsa/core/{file}`"
   - **Fix**: `mkdir -p templates/_vicevearsa/core/{dir} && cp _vicevearsa/core/{file} templates/_vicevearsa/core/{file}`
...
5. Verify `installNonMcpSkills` filters out: `vicevearsa-skill-creator`, type `mcp`, type `hybrid`, skills with `env`
...
#### Check J: IDE contamination in shared files (any `_vicevearsa/core/**` or `templates/**` changed, excluding `templates/ide-templates/**`)

Scan shared files for IDE-specific conditional logic that should live in `templates/ide-templates/{ide}/` instead.

**Files to scan:**
- All files in `_vicevearsa/core/`
...
**Exclusions:** Mentions inside this vicevearsa-dev SKILL.md itself (documentation), and any comment explicitly labeled as a cross-reference.
