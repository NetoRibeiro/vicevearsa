...
test('update preserves _memory contents', async () => {
  ...
  try {
    await init(tempDir, { _skipPrompts: true });
    await writeFile(
      join(tempDir, '_vicevearsa', '_memory', 'company.md'),
      'My Company Info',
      'utf-8'
    );
...
test('update preserves _investigations contents', async () => {
  ...
  try {
    await init(tempDir, { _skipPrompts: true });
    await writeFile(
      join(tempDir, '_vicevearsa', '_investigations', 'profile-analysis.md'),
      'investigation data',
      'utf-8'
    );
...
test('update writes new version to .vicevearsa-version', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  ...
  try {
    ...
    const version = await readFile(
      join(tempDir, '_vicevearsa', '.vicevearsa-version'),
      'utf-8'
    );
...
test('update succeeds without existing .vicevearsa-version (legacy install)', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));

  try {
    await init(tempDir, { _skipPrompts: true });
    await rm(join(tempDir, '_vicevearsa', '.vicevearsa-version'), { force: true });
...
test('update auto-imports bundled skills with env requirements', async () => {
  ...
  try {
    ...
    // Simulate a user who installed vicevearsa before this skill was bundled
