...
test('listInstalled returns empty array when skills/ does not exist', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  try {
...
test('listInstalled excludes the built-in vicevearsa-skill-creator skill', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  try {
    const skillsDir = join(dir, 'skills');
    await mkdir(join(skillsDir, 'vicevearsa-skill-creator'), { recursive: true });
...
test('installSkill copies full directory including subdirs for vicevearsa-skill-creator', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  try {
    await installSkill('vicevearsa-skill-creator', dir);
    const skill = await readFile(join(dir, 'skills', 'vicevearsa-skill-creator', 'SKILL.md'), 'utf-8');
    assert.ok(skill.length > 0);
    const scripts = await readdir(join(dir, 'skills', 'vicevearsa-skill-creator', 'scripts'));
...
test('installSkill skips copy when src and dest resolve to the same path', async () => {
  // Simulates running init from inside the vicevearsa repo itself
