...
test('init creates _vicevearsa directory structure', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));

  try {
    await init(tempDir, { _skipPrompts: true });

    await stat(join(tempDir, '_vicevearsa'));
    await stat(join(tempDir, '_vicevearsa', 'core'));
    await stat(join(tempDir, '_vicevearsa', 'core', 'architect.agent.yaml'));
    await stat(join(tempDir, '_vicevearsa', 'core', 'runner.pipeline.md'));
    await stat(join(tempDir, '_vicevearsa', '_memory'));
    await stat(join(tempDir, '.claude', 'skills', 'vicevearsa', 'SKILL.md'));
...
test('CLAUDE.md contains ViceVearsa instructions', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  ...
  try {
    ...
    assert.ok(content.includes('ViceVearsa'));
    assert.ok(content.includes('/vicevearsa'));
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
test('init creates _investigations directory', async () => {
  ...
  try {
    ...
    await stat(join(tempDir, '_vicevearsa', '_investigations'));
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test('init creates .vicevearsa-version file', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));

  try {
    await init(tempDir, { _skipPrompts: true });

    const version = await readFile(join(tempDir, '_vicevearsa', '.vicevearsa-version'), 'utf-8');
...
test('README.md contains /vicevearsa command', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
...
test('init with _ides antigravity creates .agent/rules/vicevearsa.md', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  ...
  try {
    ...
    const content = await readFile(
      join(tempDir, '.agent', 'rules', 'vicevearsa.md'),
      'utf-8'
    );
    assert.ok(content.includes('ViceVearsa'));
    assert.ok(content.includes('/vicevearsa'));
...
test('init with _ides antigravity creates .agent/workflows/vicevearsa.md', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  ...
  try {
    ...
    const content = await readFile(
      join(tempDir, '.agent', 'workflows', 'vicevearsa.md'),
      'utf-8'
    );
...
test('init creates .gitignore with browser profile exclusion', async () => {
  ...
  try {
    ...
    assert.ok(content.includes('_vicevearsa/_browser_profile/'));
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test('init creates playwright config with persistent context', async () => {
  ...
  try {
    ...
    const content = await readFile(
      join(tempDir, '_vicevearsa', 'config', 'playwright.config.json'),
      'utf-8'
    );
    const config = JSON.parse(content);
    assert.equal(config.browser.isolated, false);
    assert.equal(config.browser.userDataDir, '_vicevearsa/_browser_profile');
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test('init installs vicevearsa-skill-creator including subdirs', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  try {
    await init(tempDir, { _skipPrompts: true });
    const scripts = await readdir(join(tempDir, 'skills', 'vicevearsa-skill-creator', 'scripts'));
...
test('init with _ides codex creates .agents/skills/vicevearsa/SKILL.md', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  ...
  try {
    ...
    const content = await readFile(
      join(tempDir, '.agents', 'skills', 'vicevearsa', 'SKILL.md'),
      'utf-8'
    );
    assert.ok(content.includes('name: vicevearsa'));
    assert.ok(content.includes('description:'));
...
test('init with vscode-copilot creates .github/prompts/vicevearsa.prompt.md', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  try {
    await init(tempDir, { _skipPrompts: true, _ides: ['vscode-copilot'] });
    const content = await readFile(
      join(tempDir, '.github', 'prompts', 'vicevearsa.prompt.md'),
      'utf-8'
    );
...
test('init with cursor IDE creates .cursor/rules/vicevearsa.mdc', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'vicevearsa-test-'));
  try {
    await init(tempDir, { _skipPrompts: true, _ides: ['cursor'] });

    const content = await readFile(
      join(tempDir, '.cursor', 'rules', 'vicevearsa.mdc'),
      'utf-8'
    );
